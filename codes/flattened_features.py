...
Original file is located at
    https://colab.research.google.com/drive/1CE-9uygOZVt0shlsXD1HrYkLSyqz34YG
...
# Feature extraction function without pre-trained model
def process_and_extract_features(data):
    extracted_features = []
    for X in data:
        # Resize images to 32x32 and flatten them
        X_resized = np.array([image.img_to_array(image.array_to_img(img, scale=False).resize((100,100))).flatten() for img in X])
        extracted_features.append(X_resized)
...
class LwPClassifier:
    ...
    def fit(self, X, y, weights=None, alpha=0.01, max_iter=100, tol=1e-3):
...
for i in range(1, 21):
    ...
    if i==1:
        ...
        X_train_features = process_and_extract_features([X_train])
...
heatmap = plt.matshow(accuracy_matrix, cmap='YlGnBu', fignum=1, vmin=0, vmax=0.4) # Set color range from 0.1 to 0.3
